...
public class LFXSocketTCP extends LFXSocketGeneric {
    private final static String TAG = LFXSocketTCP.class.getSimpleName();
...
//	}
    ...
    public void connect(byte[] ipAddress, int port) {
        ...
        asyncSocket = new Thread(new TCPReceiveTask(handler), "TCPReceiveTask");
        asyncSocket.start();
    }
        ...
        public void run() {
            for (SocketMessage message : messages) {
                try {
                    ...
                    //LFXLog.d(TAG, "Message Sent Via TCP: " + MessageUnPacker.getMessageType( messageData).toString());
                } catch (Exception e) {
                    e.printStackTrace();
                    close();
                }
